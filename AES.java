...
public class AES {
    ...
    public static void main(String args[]) throws IOException {
		...
		// mode is "e" or "d" 
        String mode = args[0];
        String keyFileDir = args[1];
        String inputFileDir = args[2];

        //System.out.println(mode);
        //System.out.println(keyFileDir);
        //System.out.println(inputFileDir);
		
		// evaluate key 32 hex-characters
        BufferedReader keyreader = new BufferedReader(new FileReader(keyFileDir));
        String key = keyreader.readLine();

        BufferedReader input = new BufferedReader(new FileReader( inputFileDir ));
        ...
        if (mode.equalsIgnoreCase("e"))
        {
            ...
            FileWriter out = new FileWriter(inputFileDir + ".enc");
            String line = input.readLine();
			
			// state is actually each line, but converted into a 
			// 4x4 matrix of 2 hexadecimal characters on each 
			// cell and will be updated at each step
            int[][] state = new int[4][4];
			
			// keymatrix is a 4x44, 4x52 or 4x60 matrix that includes round keys as 4x4 matrices (depending on 128-192-256 bits of key string)
            int[][] keymatrix = aes.keySchedule(key);
			
			// processing each line in the input file
            while (line != null) {
				
                // checks should be better here, uppercase, lowercase?
                if (line.toUpperCase().matches("[0-9A-F]+"))
                {
                    
					// state is reinitialized using the next line
                    state = new int[4][4];
                    for (int i = 0; i < 4; i++)
                    {
                        for (int j = 0; j < 4; j++) {
					...
					// the first round key is added to the state
					System.out.print(MatrixToString(state) + "\n");
					aes.addRoundKey(state, aes.subKey(keymatrix, 0));
					
					// assuming that key string is 128 bit, so the steps below are repeated 9 times
					System.out.print(MatrixToString(state) + "\n");
                    for (int i = 1; i < 10; i++) {
                        aes.subBytes(state);
						System.out.print(MatrixToString(state) + "\n");
					...
					// on the last step, columns are not being mixed
                    aes.subBytes(state);
					System.out.print(MatrixToString(state) + "\n");
                    aes.shiftRows(state);
					System.out.print(MatrixToString(state) + "\n");
                    aes.addRoundKey(state, aes.subKey(keymatrix, 10));
					System.out.print(MatrixToString(state) + "\n");
					
					// convert state to a string and print it to the output file
					out.write(MatrixToString(state) + "\n");
                    line = input.readLine();
                }
				// if the line has non-hexadecimal characters, skip that line
                else
                {
                    line = input.readLine();
                }
		...
		// decrypting - steps are almost same
        else if (args[0].equalsIgnoreCase("d"))
        {
			FileWriter out = new FileWriter(inputFileDir + ".dec");
            String line = input.readLine();
            int[][] state = new int[4][4];
			int[][] keymatrix = aes.keySchedule(key);
            ...
            while (line != null) {
                if (line.toUpperCase().matches("[0-9A-F]+"))
                {
					...
					// iterating backwards on the key matrix due to decryption
					System.out.print(MatrixToString(state) + "\n");
					aes.addRoundKey(state, aes.subKey(keymatrix, 10));
	...
	// using arithmetic operations on a matrix to convert it to a single string
	...
	// get the 4x4 sub-keymatrix from the original key matrix
	...
	// left-shift the rows of a state matrix by 0-1-2-3 cells
	...
	// using subTable, update state matrix by substitute the specific cells
    public void subBytes(int[][] arr) {
        for (int i = 0; i < arr.length; i++)
        {
            for (int j = 0; j < arr[0].length; j++) {
                int temp = arr[j][i];
                arr[j][i] = subTable[temp / 16][temp % 16];
            }
        }
    }
	
	// inverted version
    public void inverseSubBytes(int[][] arr) {
        for (int i = 0; i < arr.length; i++)
        {
            for (int j = 0; j < arr[0].length; j++) {
                int temp = arr[j][i];
                arr[j][i] = invSubTable[temp / 16][temp % 16];
            }
        }
    }
    ...
    public static int[][] mul_3 ={   {0x00,0x03,0x06,0x05,0x0c,0x0f,0x0a,0x09,0x18,0x1b,0x1e,0x1d,0x14,0x17,0x12,0x11},
    ...
    public static int[][] mul_9 = {   {0x00,0x09,0x12,0x1b,0x24,0x2d,0x36,0x3f,0x48,0x41,0x5a,0x53,0x6c,0x65,0x7e,0x77},
    ...
    public static int[][] mul_11 = {  {0x00,0x0b,0x16,0x1d,0x2c,0x27,0x3a,0x31,0x58,0x53,0x4e,0x45,0x74,0x7f,0x62,0x69},
    ...
    public static int[][] mul_13 = {  {0x00,0x0d,0x1a,0x17,0x34,0x39,0x2e,0x23,0x68,0x65,0x72,0x7f,0x5c,0x51,0x46,0x4b},
    ...
    public static int[][] mul_14 = {  {0x00,0x0e,0x1c,0x12,0x38,0x36,0x24,0x2a,0x70,0x7e,0x6c,0x62,0x48,0x46,0x54,0x5a},
        ...
        while (keypoint < colsize) {
            ...
            if (temp == 0) {
                ...
                for (k = 0; k < 4; k++) {
                    ...
                    keyMatrix[k][keypoint] = subTable[hex / 16][hex % 16] ^ keyMatrix[k][keypoint - keycounter];
                }
                keypoint++;
            } else {
